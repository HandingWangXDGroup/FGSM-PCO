...
mu = torch.tensor(cifar10_mean).view(3, 1, 1).to(device_ids[0])
std = torch.tensor(cifar10_std).view(3, 1, 1).to(device_ids[0])
...
def attack_pgd(model, X, y, epsilon, alpha, attack_iters, restarts):
    max_loss = torch.zeros(y.shape[0]).to(device_ids[0])
    max_delta = torch.zeros_like(X).to(device_ids[0])
    for zz in range(restarts):
        delta = torch.zeros_like(X).to(device_ids[0])
...
def evaluate_pgd(test_loader, model, attack_iters, restarts, epsilon=(4 / 255.) / std):
    print(epsilon)
    alpha = (2 / 255.) / std
    pgd_loss = 0
    pgd_acc = 0
